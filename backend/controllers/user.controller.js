...
exports.create = async (req, res) => {
  try {
    ...
    // --- TỰ ĐỘNG GÁN GÓI DỊCH VỤ ĐANG ACTIVE CHO USER MỚI ---
    const { TenantOfferedPackage, ServiceData } = require('../models');
    const activePackages = await TenantOfferedPackage.findAll({
      where: {
    ...
    for (const pkg of activePackages) {
      const [serviceData, created] = await ServiceData.findOrCreate({
        ...
        defaults: {
          object_key: `tenant_${newUser.tenant_id}_user_${newUser.user_id}`,
          file_size: 0,
          bandwidth_used: 0,
          database_used: 0,
          api_calls_used: 0,
