...
exports.uploadFile = async (req, res) => {
  try {
    ...
    // Nếu file_storage_limit = 0 thì không giới hạn, không kiểm tra quota
    ...
    await serviceData.update({
      ...
      bandwidth_used: (serviceData.bandwidth_used || 0) + req.file.size
    });
...
exports.serveFile = async (req, res) => {
  try {
    ...
    // Thêm đoạn này trước khi update bandwidth_used
    const pathParts = decodedObjectName.split('/');
    const tenantId = parseInt(pathParts[0].split('-')[1]);
    const userId = parseInt(pathParts[1].split('-')[1]);
    const packageId = parseInt(pathParts[2].split('-')[1]);
    ...
    if (serviceData) {
      await serviceData.update({
        bandwidth_used: (serviceData.bandwidth_used || 0) + fileInfo.size
      });
    }
...
// Thêm vào cuối file, ở cấp cao nhất
exports.getAllFilesOfTenant = async (req, res) => {
  try {
    ...
    // Lấy tất cả user thuộc tenant
    const users = await User.findAll({ where: { tenant_id: parseInt(tenant_id) } });
    let allFiles = [];

    for (const user of users) {
      // Lấy tất cả service data active của user này
      const serviceDatas = await ServiceData.findAll({
        where: {
          tenant_id: parseInt(tenant_id),
          user_id: user.user_id,
          status: 'active'
      ...
      for (const sd of serviceDatas) {
        // Lấy file từ MinIO cho từng package
        const prefix = `tenant-${tenant_id}/user-${user.user_id}/package-${sd.package_id}/`;
        const files = await minioService.listFiles(prefix);

        // Lấy URL và metadata cho từng file
        for (const file of files) {
          try {
            ...
            // Nếu lỗi metadata, vẫn push file với thông tin cơ bản
            allFiles.push({
              ...file,
              fileUrl: await minioService.getFileUrl(file.name),
    ...
    res.status(200).json({ files: allFiles, totalFiles: allFiles.length });
  } catch (error) {
    console.error('Error getting all files of tenant:', error);
    res.status(500).json({ error: 'Failed to get all files of tenant', details: error.message });
  }
}; 