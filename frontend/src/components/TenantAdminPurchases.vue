      ...
      <div class="px-4 sm:px-0">
        <div class="bg-white shadow overflow-hidden sm:rounded-md">
          ...
          <div v-else-if="purchases.length === 0" class="text-center py-12">
            ...
            <p class="mt-1 text-sm text-gray-500">Bạn chưa mua dịch vụ nào cho tenant này.</p>
            <div class="mt-6">
              <button @click="goToShop" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700">
              ...
              <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                <div class="flex-1 min-w-0">
                  <div class="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                    <p><span class="font-medium">Purchase ID:</span> {{ purchase.purchase_id }}</p>
                    <p><span class="font-medium">User ID:</span> {{ purchase.user_id }}</p>
                    <p><span class="font-medium">Package ID:</span> {{ purchase.package_id }}</p>
                    <p><span class="font-medium">Tenant ID:</span> {{ purchase.tenant_id }}</p>
                    <p><span class="font-medium">Purchase Date:</span> {{ formatDate(purchase.purchase_date) }}</p>
                    <p><span class="font-medium">Start Date:</span> {{ formatDate(purchase.start_date) }}</p>
                    <p><span class="font-medium">End Date:</span> {{ formatDate(purchase.end_date) }}</p>
                    <p><span class="font-medium">Status:</span> <span :class="getStatusBadgeClass(purchase.status)">{{ getStatusText(purchase.status) }}</span></p>
                    <p><span class="font-medium">Payment ID:</span> {{ purchase.payment_id || 'N/A' }}</p>
                    <p v-if="purchase.package" class="col-span-2 md:col-span-4"><span class="font-medium">Package:</span> {{ purchase.package.name }}</p>
                  </div>
                </div>
                <div class="flex flex-col md:flex-row items-start md:items-center space-y-2 md:space-y-0 md:space-x-2 mt-2 md:mt-0">
                  <button
                  ...
                  >
                    <svg v-if="activating === purchase.purchase_id" class="animate-spin -ml-1 mr-2 h-3 w-3 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                      <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                      <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                    </svg>
                    {{ activating === purchase.purchase_id ? 'Đang kích hoạt...' : 'Kích hoạt gói dịch vụ' }}
                  </button>
                  <button
...
const activatePackage = async (purchaseId) => {
  ...
  try {
    ...
    alert('Kích hoạt gói data thành công!')
  } catch (error) {
    console.error('Error activating package:', error)
    alert('Có lỗi xảy ra khi kích hoạt gói data')
  } finally {
